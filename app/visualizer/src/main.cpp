...
#define VISUALIZER_UART_STREAM       1
#define VISUALIZER_RUN_SELECTED_MODEL_ONLY 1
#define VISUALIZER_STREAM_WAKEWORD   1
#define VISUALIZER_STREAM_KWS        0
/* Select which model output is streamed to the desktop visualizer. */
#define VISUALIZER_STREAM_MODE       VISUALIZER_STREAM_WAKEWORD
...
static void visualizer_emit_metadata(void);
static void visualizer_emit_frame(uint32_t timestamp_ms, const flt32_t* p_values, size_t values_count);
...
static const char* const WAKEWORD_VISUALIZER_LABELS[] = {
    MODEL_WAKEWORD_LABEL,
};

/*
 * Replace these placeholders with the real label order if you regenerate or retrain the KWS model.
 * The generated model exposes 10 outputs, while the demo logic below only uses a subset of them.
 */
static const char* const KWS_VISUALIZER_LABELS[] = {
    "class_0",
    "class_1",
    "class_2",
    "class_3",
...
static bool                  s_visualizer_metadata_sent;
...
int main()
{
    ...
    printk("\tWakeword: %s\n", MODEL_WAKEWORD_LABEL);
    printk("\tSupported commands: LIGHT OFF, LIGHT ON, LIGHT SWITCH\n");
...
#if !DEMO_FINAL_DETECTIONS_ONLY
#if VISUALIZER_STREAM_MODE == VISUALIZER_STREAM_WAKEWORD
    printk("Streaming wakeword model outputs to UART visualizer...\n");
#else
    printk("Streaming KWS model outputs to UART visualizer...\n");
#endif
#endif
    ...
    while (true)
    {
        ...
        if (res == NRF_EDGEAI_ERR_SUCCESS)
        {
            const flt32_t probability =
                p_wakeword_model->decoded_output.classif.probabilities.p_f32[0];
            visualizer_emit_frame(k_uptime_get_32(), &probability, 1);
        }
#else
        ...
        if (res == NRF_EDGEAI_ERR_SUCCESS)
        {
            visualizer_emit_frame(
                k_uptime_get_32(),
                p_kws_model->decoded_output.classif.probabilities.p_f32,
                sizeof(KWS_VISUALIZER_LABELS) / sizeof(KWS_VISUALIZER_LABELS[0]));
        }
#endif
...
#if VISUALIZER_UART_STREAM && (VISUALIZER_STREAM_MODE == VISUALIZER_STREAM_WAKEWORD)
                visualizer_emit_frame(k_uptime_get_32(), &probability, 1);
#endif
...
#if VISUALIZER_UART_STREAM && (VISUALIZER_STREAM_MODE == VISUALIZER_STREAM_KWS)
                visualizer_emit_frame(
                    k_uptime_get_32(),
                    p_kws_model->decoded_output.classif.probabilities.p_f32,
                    sizeof(KWS_VISUALIZER_LABELS) / sizeof(KWS_VISUALIZER_LABELS[0]));
...
{
#if VISUALIZER_STREAM_MODE == VISUALIZER_STREAM_WAKEWORD
    const char* const* p_labels = WAKEWORD_VISUALIZER_LABELS;
    const size_t       labels_count =
        sizeof(WAKEWORD_VISUALIZER_LABELS) / sizeof(WAKEWORD_VISUALIZER_LABELS[0]);
    const char* mode_name = "wakeword";
#else
    const char* const* p_labels = KWS_VISUALIZER_LABELS;
    const size_t       labels_count =
        sizeof(KWS_VISUALIZER_LABELS) / sizeof(KWS_VISUALIZER_LABELS[0]);
    const char* mode_name = "kws";
#endif

    printk("{\"type\":\"meta\",\"mode\":\"%s\",\"x_label\":\"Time (s)\",\"y_label\":\"Probability\",\"series\":[",
           mode_name);

    for (size_t index = 0; index < labels_count; ++index)
    {
        printk("%s{\"name\":\"%s\"}", index == 0 ? "" : ",", p_labels[index]);
    }
...
static void visualizer_emit_frame(uint32_t timestamp_ms, const flt32_t* p_values, size_t values_count)
{
    if (!s_visualizer_metadata_sent)
    {
        visualizer_emit_metadata();
        s_visualizer_metadata_sent = true;
    }
    ...
    const size_t emitted_values_count = (values_count < labels_count) ? values_count : labels_count;

    printk("{\"type\":\"frame\",\"timestamp_ms\":%u,\"values\":[", (unsigned int)timestamp_ms);

    for (size_t index = 0; index < emitted_values_count; ++index)
    {
        printk("%s%.5f", index == 0 ? "" : ",", (double)p_values[index]);
    }

    printk("],\"count\":%u}\n", (unsigned int)emitted_values_count);
}
...
static bool is_keyword_phrase_first_word(uint16_t predicted_class)
{
    return predicted_class == KEYWORD_LIGHT;
}
...
static bool is_keyword_phrase_second_word(uint16_t predicted_class)
{
    return (predicted_class == KEYWORD_OFF) || (predicted_class == KEYWORD_ON) ||
...
static bool is_valid_keyword_command_pair(uint16_t first_class, uint16_t second_class)
{
    return (first_class == KEYWORD_LIGHT) &&
           ((second_class == KEYWORD_OFF) || (second_class == KEYWORD_ON) ||
            (second_class == KEYWORD_SWITCH));
}
