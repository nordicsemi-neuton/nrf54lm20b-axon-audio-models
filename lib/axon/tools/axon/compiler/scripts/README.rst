...
.. list-table:: Variant Parameters
   ...
   * - DISABLE_OP_QUANTIZATION
     - BOOL
     - Flag used to disable the output quantization. The output of a model is often relatively small. 
       Disabling output quantization for classification models can help determine if quantization at the last layer is affecting model accuracy.
       Users can choose to disable quantization of the output if they prefer to retain the precision of the unquantized (32bit) output.
   ...
   * - RUN_ALL_VARIANTS
     - BOOL
     - When ``true``, overrides the variant parameters to run all possible variant combinations for the model. 
...
Steps to Add a CPU Extension Operation in the Executor
****************************************************************

Step 1: Add Enum to Header File
--------------------------------
- Add the CPU extension operation to the ``AxonproOperationEnum`` in the ``nrf_axon_nn_compiler_types.h`` file.
- Record the enum string for later use.

Step 2: Define the Python Class
--------------------------------
- Create a class for the cpu extension operation in the ``cpu_operations.py`` file.
- The class should inherit from the ``CpuExtension`` class.

Step 3: Implement Required APIs
--------------------------------
- Implement the following APIs for the CPU extension operation:

  **Step 3.1: HandleOperatorOptions**
    - Populate different operator options using the ``SET/GET`` methods provided by the operator object.

    - Calculate or Update the following fields as needed:

      * Scale shifts and multipliers (use the functions in util to calculate, or write your own)
      * Filter tensor
      * Input/output bitwidth
      * Input/output zeropoints
      * ``b_prime`` tensor (if needed)
      * Activation functions 
      * Padding and Stride values
      * Other Attributes as necessary using the SET/GET methods.


  **Step 3.2: HandlePreviousOperatorAttributes**
    - Modify scales, activation, and other attributes in the previous operation to correctly implement the CPU operation. if nothing is neeeded to be modified, implement this as a *pass* function.

  **Step 3.3: Add Extra CPU attributes**
    - Populate additional CPU op attributes if needed using the ``FillAdditionalCpuAttributes`` API from the operator options object.

Step 4: Register the Class
---------------------------
- Add the class to the ``cpu_operators_list`` in the ``cpu_operator_options.py`` file.
- Use the TFLite operator code as the key to add the CPU extension op.
- Pass the enum string (defined in ``nrf_axon_nn_compiler_types.h``) when initializing the object in the dictionary.

Step 5: Implement the Operation in C in the compiler
------------------------------
- Implement the CPU extension operation in C in the axon compiler.

Step 6: Verify
------------------------------
- Verify that the operation works correctly on hardware and is generating the expected results
